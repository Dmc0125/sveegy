...
const {
  id, message, status, hideNotification,
} = useNotification()
</script>
  ...
  >
    ...
    >
      <div
        ...
        :class="status === 'error' ? 'notification__svg--error' : 'notification__svg--success'"
      >
        <svg-wrapper :icon="status === 'error' ? 'close-icon' : 'tick-icon'" />
      </div>
